...
def module_card(icon, title, desc):
    """Kartu modul (murni tampilan). Aksi buka modul memakai st.button +
    st.switch_page di bawah kartu — navigasi internal Streamlit sehingga
    session_state (peran & dataset) TIDAK hilang. Tautan <a href> lama
    menyebabkan muat-ulang penuh: sesi baru tanpa peran -> halaman tujuan
    tidak terdaftar di navigasi -> error 'Page not found'."""
    return (f'<div class="menu-card" style="cursor:default;min-height:188px;">'
...
# ====================== SUDAH MEMILIH PERAN: kartu modul ======================
    ...
    if role == "pemilik":
        ...
        modules = [
            (IC["data"], "Lihat Dataset",
             "Unggah data POS (CSV/XLSX), jelajahi insight lewat panel kontrol, "
             "lalu unduh hasil sesuai filter.", "app_pages/p2_lihat_dataset.py"),
            (IC["pie"], "Klaster Menu",
             "Segmentasi menu makanan &amp; minuman dengan K-Means++. Kontrol "
             "klaster, lihat &amp; unduh hasilnya.", "app_pages/p3_klaster_menu.py"),
            (IC["receipt"], "Klaster Transaksi",
             "Segmentasi nota/transaksi dengan K-Means++. Telusuri profil segmen, "
             "rincian kategori, lalu unduh.", "app_pages/p4_klaster_transaksi.py"),
        ]
    else:  # developer
        nama_peran = "Pengembang / Developer"
        ajakan = "Pilih salah satu modul teknis di bawah untuk memulai."
        modules = [
            (IC["chart"], "Developer &middot; Menu",
             "Eksperimen teknis lengkap: metrik Elbow/Silhouette/DBI, PCA, "
             "heatmap korelasi, dan pemilihan kombinasi fitur.", "app_pages/p5_dev_menu.py"),
            (IC["chart2"], "Developer &middot; Transaksi",
             "Eksperimen teknis klaster transaksi: Elbow/Silhouette/DBI, PCA "
             "2D/3D, korelasi, dan pemilihan kombinasi fitur.", "app_pages/p6_dev_transaksi.py"),
        ]
    ...
    for i, (col, (icon, title, desc, target)) in enumerate(zip(cols, modules)):
        with col:
            _md(module_card(icon, title, desc))
            if st.button("Buka modul", key=f"open_module_{i}",
                         type="primary", use_container_width=True):
